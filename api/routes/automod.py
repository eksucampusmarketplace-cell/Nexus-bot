...
async def update_advanced_automod(chat_id: int, request: Request):
    ...
    for rule_key, tw in body.get("time_windows", {}).items():
        await set_rule_time_window(db, chat_id, rule_key, tw["start"], tw["end"])

    # Penalties
    for rule_key, p in body.get("penalties", {}).items():
        await set_rule_penalty(db, chat_id, rule_key, p["penalty"], p.get("duration_hours", 0))
...
async def get_templates(chat_id: int, request: Request):
    db = request.app.state.db
    _check_access(request, chat_id)
    rows = await db.fetch("SELECT * FROM rule_templates ORDER BY is_builtin DESC, name ASC")
...
async def apply_template(chat_id: int, request: Request):
    ...
    row = await db.fetchrow("SELECT * FROM rule_templates WHERE id=$1", template_id)
    if not row:
