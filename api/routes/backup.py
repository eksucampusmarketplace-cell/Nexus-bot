...
async def export_backup(chat_id: int, user: dict = Depends(get_current_user)):
    ...
    async with db.pool.acquire() as conn:
        # Get group settings
        group_row = await conn.fetchrow("SELECT * FROM groups WHERE chat_id = $1", chat_id)
        if not group_row:
        ...
        webhook_rows = await conn.fetch(
            "SELECT url, events, secret FROM webhooks WHERE chat_id = $1", chat_id
        )
        webhooks = [dict(row) for row in webhook_rows]

        # Get log channel
        log_channel_row = await conn.fetchrow(
            "SELECT channel_id FROM log_channels WHERE chat_id = $1", chat_id
        )
...
async def import_backup(chat_id: int, request: Request, user: dict = Depends(get_current_user)):
    ...
    if not is_valid:
        ...
        raise HTTPException(400, f"Invalid backup data: {error_messages}")
    ...
    async with db.pool.acquire() as conn:
        # Verify group exists
        exists = await conn.fetchval("SELECT 1 FROM groups WHERE chat_id = $1", chat_id)
        if not exists:
            raise HTTPException(404, "Group not found")
