...
from db.ops.bots_messages import (
    get_bot_custom_messages,
    get_bot_custom_message,
    set_bot_custom_message,
    delete_bot_custom_message,
)
...
async def list_bot_messages(bot_id: int, user: dict = Depends(get_current_user)):
    ...
    for key in DEFAULTS:
        ...
        result[key] = {"body": body if body else DEFAULTS[key], "isCustom": key in custom_messages}
...
async def update_bot_message(
    bot_id: int, key: str, payload: dict, user: dict = Depends(get_current_user)
):
