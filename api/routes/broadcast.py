...
from db.ops.broadcast import create_broadcast_task, get_broadcast_task, get_bot_broadcasts, get_broadcast_targets
...
async def start_broadcast(
    ...
    task_id = await create_broadcast_task(db.pool, {
        "owner_id": user["id"],
        "bot_id": bot_id,
...
async def start_global_broadcast(
    ...
    async with db.pool.acquire() as conn:
        ...
        if primary_bot:
            ...
            if targets:
                tid = await create_broadcast_task(db.pool, {
                    "owner_id": user["id"],
        ...
        if include_clones:
            clones = await conn.fetch("SELECT bot_id FROM bots WHERE is_primary = FALSE AND status = 'active'")
            for clone in clones:
