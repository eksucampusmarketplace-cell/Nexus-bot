...
async def link_channel_route(group_chat_id: int, body: dict, user: dict = Depends(get_current_user)):
...
async def immediate_post(group_chat_id: int, body: dict, user: dict = Depends(get_current_user)):
    ...
    async with db.pool.acquire() as conn:
        ...
        """, info["bot_id"], info["channel_id"], group_chat_id, text, datetime.now(timezone.utc), msg.message_id, user['id'])
