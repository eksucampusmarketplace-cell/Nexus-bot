...
async def list_federations(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            "federations": [dict(r) for r in rows],
            "count": len(rows)
        }
...
@router.post("/federations/{fed_id}/regenerate-code")
    ...
    try:
        async with db.pool.acquire() as conn:
            ...
            result = await conn.execute(
                ...
                new_code, fed_id, user_id
            )
            
            if result == "UPDATE 0":
                raise HTTPException(status_code=403, detail="Not authorized or federation not found")
...
@router.get("/groups/{chat_id}/federations")
...
async def get_personality_preview(
    tone: str = "neutral",
    language: str = "en",
    emoji: bool = True
):
...
async def get_available_tones():
    ...
    return {
        "tones": [
            {"id": tone, "description": data["description"]}
            for tone, data in TONES.items()
        ]
    }
...
@router.get("/groups/{chat_id}/personality")
...
@router.put("/groups/{chat_id}/personality")
