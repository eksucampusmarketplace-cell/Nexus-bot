...
async def get_user_telegram_status(bot, chat_id: int, user_id: int) -> str:
    ...
    try:
        ...
        logger.warning(f"[ME] Failed to get chat member status | chat_id={chat_id} user_id={user_id}: {e}")
        return 'unknown'
...
async def get_user_context(user: dict = Depends(get_current_user)):
    ...
    for group_row in all_groups:
        ...
        # Ensure group_settings is a dict
    ...
    response = {
        ...
        "bot_info": bot_info,
        "support_group": settings.SUPPORT_GROUP_URL if hasattr(settings, 'SUPPORT_GROUP_URL') else None
    }

    logger.info(f"[ME] User context ready | user_id={user_id} role={role} "
...
async def get_member_channel_status(group_id: int, user_id: int) -> dict:
    ...
    return {
        ...
        "is_restricted": record['is_restricted'] if record else False,
        "channel_invite": f"t.me/{config.get('force_channel_username')}" if config.get('force_channel_username') else None
    }
