...
async def get_member_group_stats(chat_id: int, user: dict = Depends(get_current_user)):
    """
    ...
    if user_row and user_row["warns"]:
        ...
        warns = (
            user_row["warns"]
            if isinstance(user_row["warns"], list)
            else json.loads(user_row["warns"])
        )
    ...
    return {
        ...
        "boost": {
            ...
            "current": (
                (boost_row["invited_count"] + boost_row["manual_credits"]) if boost_row else 0
            ),
        ...
        "join_date": (
            user_row["join_date"].isoformat() if user_row and user_row["join_date"] else None
        ),
    }
...
async def get_member_group_rankings(
    ...
    return {
        ...
        "you": {"rank": user_rank or 0, "value": user_total},
    }


@router.get("/groups/{chat_id}/invitelink")
async def get_member_invite_link(chat_id: int, user: dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        result = await bot_app.bot.create_chat_invite_link(
            chat_id=chat_id, name=invite_name, creates_join_request=False
        )
        ...
        logger.error(
            f"[ME] Failed to create invite link | chat_id={chat_id} user_id={user_id}: {e}"
        )
        raise HTTPException(status_code=500, detail="Failed to create invite link")


@router.get("/groups/{chat_id}/rules")
async def get_group_rules(chat_id: int, user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "rules_text": (
            ...
            if rules
            else "No rules set for this group."
        ),
    }


@router.get("/groups/{chat_id}/activity")
async def get_member_activity(chat_id: int, user: dict = Depends(get_current_user)):
    """
    ...
    for i in range(6, -1, -1):
        ...
        days.append({"date": date_str, "day": date.strftime("%a"), "messages": messages})
