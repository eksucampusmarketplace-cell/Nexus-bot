...
async def update_scheduled(chat_id: int, msg_id: int, request: Request):
    ...
    allowed = {
        "content", "max_sends", "pin_after_send", "interval_mins",
        "cron_expr", "days_of_week", "time_of_day", "timezone", "is_active"
    }
    ...
    await db.execute(
        f"UPDATE scheduled_messages SET {set_clauses} WHERE id=$1 AND chat_id=$2",
        *values
    )
    return {"ok": True}
...
async def pause_scheduled(chat_id: int, msg_id: int, request: Request):
    db  = request.app.state.db
    row = await db.fetchrow(
        "SELECT is_active FROM scheduled_messages WHERE id=$1 AND chat_id=$2",
        msg_id, chat_id
    )
    if not row:
...
async def update_report(chat_id: int, report_id: int, request: Request):
    ...
    await db.execute(
        ...
        status, user_id, report_id, chat_id
    )
    return {"ok": True}
