...
async def list_webhooks(chat_id: int, user: dict = Depends(get_current_user)):
    ...
    return {"webhooks": webhooks, "stats": stats}


@router.post("/{chat_id}/webhooks")
async def create_webhook(chat_id: int, data: WebhookCreate, user: dict = Depends(get_current_user)):
    """Create a new webhook configuration."""
    # Validate event types
    invalid_events = [e for e in data.events if e not in VALID_EVENT_TYPES]
    if invalid_events:
        raise HTTPException(
            status_code=400, detail=f"Invalid event types: {', '.join(invalid_events)}"
        )
...
async def get_webhook(chat_id: int, webhook_id: int, user: dict = Depends(get_current_user)):
...
async def update_webhook(
    chat_id: int, webhook_id: int, data: WebhookUpdate, user: dict = Depends(get_current_user)
):
...
async def delete_webhook(chat_id: int, webhook_id: int, user: dict = Depends(get_current_user)):
...
async def test_webhook(
    chat_id: int, webhook_id: int, data: WebhookTest, user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        return {"success": False, "error": "Timeout", "duration_ms": duration_ms}
...
async def get_webhook_deliveries(
    chat_id: int, webhook_id: int, limit: int = 50, user: dict = Depends(get_current_user)
):
...
async def list_event_types(user: dict = Depends(get_current_user)):
    ...
    return {
        "events": [
            ...
            {
                "type": "member_leave",
                "description": "Triggered when a member leaves or is removed",
                "icon": "👋",
            },
            ...
            {
                "type": "warn",
                "description": "Triggered when a member receives a warning",
                "icon": "⚠️",
            },
            {"type": "kick", "description": "Triggered when a member is kicked", "icon": "👢"},
            {
                "type": "message",
                "description": "Triggered on every message (use with caution)",
                "icon": "💬",
            },
            {
                "type": "automod_trigger",
                "description": "Triggered when AutoMod takes action",
                "icon": "🤖",
            },
            {
                "type": "report_created",
            ...
            },
            {
                ...
                "description": "Triggered when group settings are modified",
                "icon": "⚙️",
            },
