...
async def check_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> AutomodResult:
    ...
    for rule_key in ordered_rules:
        ...
        if tw:
            in_window = is_in_time_window(now_time, tw["start_time"], tw["end_time"])
            if not in_window:
                continue  # lock only active during window, not now

        # Check timed lock
        tl = timed_locks.get(rule_key)
        if tl and not is_in_time_window(now_time, tl["start"], tl["end"]):
            continue
    ...
    if dup_limit > 0 and msg.text:
        msg_hash = hashlib.md5(msg.text.strip().lower().encode()).hexdigest()

        count = await get_recent_hash_count(db, chat.id, msg_hash, dup_window)
        if count >= dup_limit:
            result = AutomodResult(
                violated=True,
                rule_key="duplicate",
...
async def _enforce(msg, user, chat, context, settings, result: AutomodResult, duration_hours: int):
    ...
    if penalty in ("silence", "kick", "ban"):
        await apply_penalty(bot, chat.id, user.id, PenaltyType(penalty), duration_hours)
...
async def _send_admonition(msg, user, chat, context, settings, result: AutomodResult):
...
async def _record_warning(context, chat_id, user_id, settings):
    ...
    await db.execute(
        "INSERT INTO user_warnings (chat_id, user_id) VALUES ($1,$2)", chat_id, user_id
    )
    count += 1
...
async def _get_owner_id(db, bot_id: int) -> int | None:
    if not db:
        return None
    row = await db.fetchrow("SELECT owner_id FROM clone_bots WHERE bot_id=$1", bot_id)
