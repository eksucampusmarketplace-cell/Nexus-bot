        ...
        async with pool.acquire() as conn:
            ...
            if not is_admin and amount > 0:
                ...
                daily_row = await conn.fetchrow(
                    ...
                    """,
                    chat_id, from_user_id, bot_id, today
                )
                ...
                if current_given >= DEFAULT_DAILY_LIMIT:
                    ...
                    return False, f"You've reached your daily limit. You have {remaining} rep left to give today."
            ...
            await conn.execute(
                ...
                """,
                chat_id, from_user_id, bot_id, abs(amount)
            )
            ...
            await conn.execute(
                ...
                """,
                chat_id, to_user_id, bot_id, amount
            )
            ...
            to_row = await conn.fetchrow(
                ...
                """,
                chat_id, to_user_id, bot_id
            )

        log.info(
            f"[REP] {from_user_id} gave {amount} rep to {to_user_id} in chat {chat_id}"
        )
    ...
    try:
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                WHERE chat_id=$1 AND user_id=$2 AND bot_id=$3 AND date=$4
                """,
                chat_id, user_id, bot_id, date.today()
            )
