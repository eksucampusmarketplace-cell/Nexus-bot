...
async def cmd_addfilter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if keyword in filters_list:
        await update.message.reply_text(f"❌ Filter '<code>{keyword}</code>' already exists", parse_mode=ParseMode.HTML)
        return
...
async def cmd_delfilter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if keyword not in filters_list:
        await update.message.reply_text(f"❌ Filter '<code>{keyword}</code>' not found", parse_mode=ParseMode.HTML)
        return
