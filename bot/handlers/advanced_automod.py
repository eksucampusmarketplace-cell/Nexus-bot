...
from db.ops.automod import (
    set_rule_time_window, remove_rule_time_window,
    set_rule_penalty, upsert_silent_time, disable_silent_time,
    clear_all_silent_times, update_group_setting,
    add_necessary_word, remove_necessary_word, clear_necessary_words,
    add_regex_pattern, remove_regex_pattern, get_regex_patterns,
    set_timed_lock, remove_timed_lock
)
...
VALID_LOCKS = {
    "link", "website", "username", "photo", "video", "sticker",
    "gif", "forward", "forward_channel", "text", "voice", "file",
    "software", "poll", "slash", "no_caption", "emoji_only",
    "emoji", "game", "english", "arabic_farsi", "reply",
    "external_reply", "bot", "unofficial_tg", "hashtag",
    "location", "phone", "audio", "spoiler"
}
VALID_PENALTIES = {"delete", "silence", "kick", "ban"}


async def handle_automod_command(
    update: Update,
    ...
    m = re.match(
        r'lock (\w+) from (\d{1,2}(?::\d{2})?) to (\d{1,2}(?::\d{2})?)',
        cmd
    )
    if m:
        rule_key = m.group(1)
        ...
        if rule_key in VALID_LOCKS:
            ...
            await msg.reply_text(
                f"⏰ Lock <b>{rule_key}</b> active from "
                f"{start} to {end}",
                parse_mode="HTML"
            )
        return
    ...
    if m:
        ...
        if rule_key in VALID_LOCKS and penalty in VALID_PENALTIES:
            ...
            await msg.reply_text(
                f"⚖️ Violation of <b>{rule_key}</b> → "
                f"<b>{penalty}</b>{dur_str}",
                parse_mode="HTML"
            )
        return
    ...
    m = re.match(
        r'(first|second|third) silent time from (\d{1,2}(?::\d{2})?) to (\d{1,2}(?::\d{2})?)',
        cmd
    )
    if m:
    ...
    if m:
        ...
        await msg.reply_text(
            f"📋 Duplicate limit: {'disabled' if n == 0 else n}"
        )
        return
    ...
    for pattern, (setting_key, label) in COUNT_CMDS.items():
        ...
        if m:
            ...
            await msg.reply_text(
                f"📏 {label}: {'disabled' if n == 0 else n}"
            )
            return
    ...
    if m:
        ...
        try:
            ...
            await msg.reply_text(f"🔍 REGEX added: <code>{pattern}</code>",
                                  parse_mode="HTML")
    ...
    if m:
        ...
        await msg.reply_text(f"📝 Required word added: <b>{word}</b>",
                              parse_mode="HTML")
        return
    ...
    if cmd.startswith("!be.in.text"):
        ...
        await msg.reply_text(f"✅ Required word removed: <b>{word}</b>",
                              parse_mode="HTML")
        return
    if cmd == "beintexts":
    ...
    if m:
        ...
        await msg.reply_text(
            f"⏰ <b>{rule_key}</b> locked from {start} to {end}",
            parse_mode="HTML"
        )
        return
