...
async def handle_new_member(bot, update: Update, config: dict, is_channel_gate: bool = False):
    ...
    if config.get("force_add_enabled") and config.get("force_add_required", 0) > 0:
        await handle_boost_join(bot, chat, new_user, member, join_source, config, record)
...
async def notify_admins_manual_add(bot, chat, new_user, added_by_id, config: dict):
    ...
    keyboard = [
        [InlineKeyboardButton("👤 Assign Credit", callback_data=f"booster:assign:{new_user.id}")]
    ]
...
async def send_witness_dm(bot, new_user, chat, config: dict):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "❓ I don't know", callback_data=f"witness:unknown:{chat.id}:{new_user.id}"
                )
            ],
            [
                InlineKeyboardButton(
...
async def handle_channel_gate(bot, chat, new_user, config: dict):
    ...
    from db.ops.booster import create_channel_record, set_channel_restricted, get_channel_record
    ...
    await create_channel_record(
        group_id=chat.id, user_id=new_user.id, username=new_user.username, channel_id=channel_id
    )
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ I Joined", callback_data=f"channel:verify:{chat.id}:{new_user.id}"
                )
            ]
        ]
    )
    ...
    try:
        await bot.send_message(new_user.id, message_template, reply_markup=keyboard)
    except (TelegramError, Forbidden):
        pass
...
async def boost_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not record:
        await update.message.reply_text("No boost requirement is active for you in this group.")
        return
...
async def remain_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not record or record.get("is_unlocked"):
        await update.message.reply_text("✅ You've completed your boost! No more invites needed.")
        return
...
async def extract_user_from_args(update: Update, context) -> Optional[object]:
    ...
    if arg.startswith("@"):
        ...
        await update.message.reply_text("Please reply to a user's message to mention them.")
        return None
