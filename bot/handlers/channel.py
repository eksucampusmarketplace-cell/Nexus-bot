...
async def send_to_channel(
    ...
    logger.info(
        f"[CHANNEL] Posted | channel_id={channel_id} | type={media_type or 'text'} | msg_id={msg.message_id}"
    )
    return msg
...
async def copy_message_to_channel(bot, channel_id: int, from_chat_id: int, message_id: int):
    ...
    return await bot.copy_message(
        chat_id=channel_id, from_chat_id=from_chat_id, message_id=message_id
    )
...
async def channel_post_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT channel_id FROM linked_channels WHERE group_chat_id = $1",
            update.effective_chat.id,
        )
        if not row:
            await update.message.reply_text(
                "❌ No channel linked. Use the Mini App to link a channel."
            )
            return
...
async def cancel_post_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "UPDATE channel_posts SET status = 'cancelled' WHERE id = $1 AND group_chat_id = $2",
            post_id,
            update.effective_chat.id,
        )
...
async def edit_post_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT channel_id, sent_message_id FROM channel_posts WHERE id = $1 AND group_chat_id = $2",
            post_id,
            update.effective_chat.id,
        )
        if not row:
        ...
        if row["sent_message_id"]:
            ...
            await conn.execute(
                "UPDATE channel_posts SET text = $1 WHERE id = $2", new_text, post_id
            )
    ...
    async with db_pool.acquire() as conn:
        ...
        if row["sent_message_id"]:
            await context.bot.delete_message(
                chat_id=row["channel_id"], message_id=row["sent_message_id"]
            )
            ...
            await conn.execute(
                "UPDATE channel_posts SET status = 'cancelled' WHERE id = $1", post_id
            )
