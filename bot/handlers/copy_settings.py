...
async def cmd_copy_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ...
    src_settings = await db.fetchrow("SELECT * FROM groups WHERE chat_id=$1", chat.id)
    if not src_settings:
    ...
    copy_data = {
        k: src_dict[k] for k in COPYABLE_SETTINGS_KEYS if k in src_dict and src_dict[k] is not None
    }

    set_clauses = ", ".join(f"{k}=${i+2}" for i, k in enumerate(copy_data.keys()))
    ...
    silent_rows = await db.fetch(
        "SELECT * FROM silent_times WHERE chat_id=$1 AND is_active=TRUE", chat.id
    )
