...
async def send_welcome_in_group(update, context, user, chat, processed_welcome, media=None):
    try:
        if media:
            ...
            if media_type == 'photo':
                await context.bot.send_photo(chat_id=chat.id, photo=file_id, caption=processed_welcome, parse_mode=ParseMode.HTML)
            elif media_type == 'video':
                await context.bot.send_video(chat_id=chat.id, video=file_id, caption=processed_welcome, parse_mode=ParseMode.HTML)
            elif media_type == 'animation':
                await context.bot.send_animation(chat_id=chat.id, animation=file_id, caption=processed_welcome, parse_mode=ParseMode.HTML)
        else:
            await context.bot.send_message(chat_id=chat.id, text=processed_welcome, parse_mode=ParseMode.HTML)
        
        logger.info(f"[WELCOME] Sent | chat_id={chat.id} | user_id={user.id} | mode=group | media={'photo' if media else 'none'}")
...
async def welcome_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow("SELECT modules, text_config FROM groups WHERE chat_id = $1", chat.id)
        if not row:
            return
        ...
        if isinstance(text_config, str): text_config = json.loads(text_config)
        ...
        if send_as_dm:
            try:
                ...
                notice = await update.message.reply_text(
                    f"👋 Welcome {user.mention_html()}! Check your DMs.",
                    parse_mode=ParseMode.HTML
                )
...
async def goodbye_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        ...
        modules = row['modules'] or {}
        if isinstance(modules, str): modules = json.loads(modules)
        if not modules.get('goodbye_message', False): return
    ...
    await context.bot.send_message(chat_id=chat.id, text=processed_goodbye, parse_mode=ParseMode.HTML)
...
async def set_welcome_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db_pool = context.bot_data["db_pool"]
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow("SELECT text_config FROM groups WHERE chat_id = $1", update.effective_chat.id)
        config = row['text_config'] or {}
        if isinstance(config, str): config = json.loads(config)
        config['welcome'] = text
        if media: config['welcome_media'] = media
        await conn.execute("UPDATE groups SET text_config = $1 WHERE chat_id = $2", json.dumps(config), update.effective_chat.id)
