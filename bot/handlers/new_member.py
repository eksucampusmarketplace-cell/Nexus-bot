...
async def handle_chat_member_update(
    ...
    if new_status == "member" and old_status in ("left", "kicked", "restricted"):
        ...
        if user.is_bot and settings.get("bot_inviter_ban"):
            ...
            if inviter:
                try:
                    ...
                    log.info(
                        f"[NEW_MEMBER] Bot inviter banned | "
                        f"inviter={inviter.id} chat={chat.id}"
                    )
                except Exception:
                    pass
            return
        ...
        # Anti-raid check
        restricted = await handle_new_member(
            context.bot, chat.id, user, settings, db, owner_id
        )
        ...
        if settings.get("captcha_enabled") or (restricted and settings.get("antiraid_mode") == "captcha"):
            ...
            join_msg_id = None
            await send_captcha(
                context.bot, chat.id, user, settings, db, join_msg_id
            )
            return
    ...
    # ── Member left ────────────────────────────────────────────────────────
        ...
        await log_member_event(
            db, chat.id, user,
            "leave" if new_status == "left" else "kick"
        )
