...
async def track_name_change_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with db.acquire() as conn:
            optout = await conn.fetchval(
                "SELECT 1 FROM user_history_optout WHERE user_id = $1", new_user.id
            )
            if optout:
                return
    except Exception:
    ...
    try:
        async with db.acquire() as conn:
            ...
            log.debug(
                f"[SANGMATA] ChatMember change detected for {new_user.id}: {'; '.join(changes)}"
            )
            ...
            change_type = (
                ...
                if (
                    old_user.first_name == new_user.first_name
                    and old_user.last_name == new_user.last_name
                )
                else "name"
            )
...
async def _sync_to_federation(
    ...
    try:
        ...
        fed_ids = await conn.fetch(
            "SELECT federation_id FROM federation_members WHERE chat_id = $1", source_chat_id
        )
        ...
        for fed in fed_ids:
            ...
            for member in member_chats:
                ...
                target_enabled = await conn.fetchval(
                    "SELECT name_history_enabled FROM groups WHERE chat_id = $1", target_chat_id
                )
                if target_enabled:
...
async def cmd_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with db.acquire() as conn:
            ...
            # Check optout status
            optout = await conn.fetchval(
                "SELECT 1 FROM user_history_optout WHERE user_id = $1", target_id
            )
...
async def cmd_historyoptin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with db.acquire() as conn:
            await conn.execute("DELETE FROM user_history_optout WHERE user_id = $1", user.id)
