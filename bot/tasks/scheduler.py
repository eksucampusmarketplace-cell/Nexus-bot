                ...
                try:
                    ...
                    async with db_pool.acquire() as conn:
                        await conn.execute("""
                            ...
                            WHERE id = $3
                        """, datetime.now(timezone.utc), msg.message_id, post["id"])
                    
                    logger.info(f"[SCHEDULER] Sent scheduled post | post_id={post['id']} | channel={post['channel_id']}")

                except Exception as e:
                    logger.error(f"[SCHEDULER] Failed to send post_id={post['id']}: {e}", exc_info=True)
                    async with db_pool.acquire() as conn:
                        await conn.execute("""
                            UPDATE channel_posts 
