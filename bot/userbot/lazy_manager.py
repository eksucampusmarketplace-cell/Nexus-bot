...
class LazyClientManager:
    ...
    async def _load_client(self, userbot_id: int) -> Client:
        ...
        # Note: api_id and api_hash are not stored in DB - always use config values
        row = await self.pool.fetchrow(
            "SELECT session_string, tg_name FROM music_userbots "
        ...
        # Decrypt the session string before passing to Pyrogram
        try:
            session_string = decrypt_token(row["session_string"])
        except Exception as e:
            logger.error(f"[LAZY_MANAGER] Failed to decrypt session for userbot {userbot_id}: {e}")
            raise ValueError(f"Failed to decrypt session for userbot {userbot_id}: {e}")

        # Always use API credentials from config
        api_id = settings.PYROGRAM_API_ID
        api_hash = settings.PYROGRAM_API_HASH

        if not api_id or not api_hash:
            raise ValueError(f"Missing PYROGRAM_API_ID or PYROGRAM_API_HASH in config")
        ...
        await client.start()
        logger.info(f"[LAZY_MANAGER] Loaded userbot {userbot_id} ({row.get('tg_name', 'Unknown')})")
        return client
    ...
    async def cleanup_inactive_clients(self, force: bool = False) -> int:
        ...
        async with self._lock:
            if force:
                ...
                to_unload = [
                    uid
                    for uid, last in self.last_used.items()
