...
class BroadcastEngine:
    ...
    async def _run_broadcast(self, task_id: int):
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE broadcast_tasks SET total_targets = $1 WHERE id = $2", len(targets), task_id
            )
        ...
        logger.info(
            f"[BROADCAST] Starting task {task_id} for bot {bot_id} to {len(targets)} targets"
        )
                ...
                if media_type and media_file_id:
                    if media_type == "photo":
                        await bot.send_photo(
                            chat_id, media_file_id, caption=content, parse_mode="HTML"
                        )
