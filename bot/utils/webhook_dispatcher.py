...
async def dispatch_event(chat_id: int, event_type: str, data: Dict[str, Any], db_pool=None):
    """
    ...
    try:
        ...
        logger.debug(
            f"[WEBHOOK] Dispatching {event_type} to {len(webhooks)} webhooks for chat {chat_id}"
        )
...
async def _send_to_webhooks(webhooks: list, event_type: str, data: Dict[str, Any]):
    """Send event to multiple webhooks concurrently."""
    tasks = [_send_single_webhook(webhook, event_type, data) for webhook in webhooks]
    await asyncio.gather(*tasks, return_exceptions=True)


async def _send_single_webhook(webhook: Dict[str, Any], event_type: str, data: Dict[str, Any]):
...
async def notify_member_leave(
    chat_id: int, user_id: int, chat_title: str, reason: Optional[str] = None, **kwargs
):
