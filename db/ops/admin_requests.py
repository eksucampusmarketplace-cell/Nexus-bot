...
async def create_admin_request(
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            message_id,
            message_text[:4000],
            reply_to_msg_id,  # Truncate to 4000 chars
        )
...
async def get_request(pool, request_id: int) -> Optional[dict]:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM admin_requests WHERE id = $1", request_id)
...
async def get_user_recent_request_count(
    pool, chat_id: int, user_id: int, period_seconds: int = 3600
) -> int:
...
async def get_group_request_stats(pool, chat_id: int) -> dict:
    ...
    async with pool.acquire() as conn:
        total = await conn.fetchval(
            "SELECT COUNT(*) FROM admin_requests WHERE chat_id = $1", chat_id
        )
        open_count = await conn.fetchval(
            "SELECT COUNT(*) FROM admin_requests WHERE chat_id = $1 AND status = 'open'", chat_id
        )
        closed_count = await conn.fetchval(
            "SELECT COUNT(*) FROM admin_requests WHERE chat_id = $1 AND status = 'closed'", chat_id
        )
...
async def get_group_setting(pool, chat_id: int, setting: str) -> any:
    ...
    async with pool.acquire() as conn:
        value = await conn.fetchval(f"SELECT {setting} FROM groups WHERE chat_id = $1", chat_id)
    return value
...
async def set_group_setting(pool, chat_id: int, setting: str, value: any) -> bool:
    ...
    try:
        async with pool.acquire() as conn:
            await conn.execute(
                f"UPDATE groups SET {setting} = $1 WHERE chat_id = $2", value, chat_id
            )
        return True
    except Exception:
        return False
