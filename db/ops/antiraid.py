...
async def record_join(db, chat_id: int, user_id: int):
    await db.execute(
        "INSERT INTO recent_joins (chat_id, user_id) VALUES ($1, $2)", chat_id, user_id
    )
...
async def create_antiraid_session(
    db, chat_id: int, triggered_by: str, ends_at=None, join_count: int = 0
):
    row = await db.fetchrow(
...
async def get_active_session(db, chat_id: int):
    return await db.fetchrow(
        "SELECT * FROM antiraid_sessions WHERE chat_id = $1 AND is_active = TRUE LIMIT 1", chat_id
    )


async def end_antiraid_session(db, session_id: int):
    await db.execute("UPDATE antiraid_sessions SET is_active = FALSE WHERE id = $1", session_id)


async def increment_session_joins(db, session_id: int):
    await db.execute(
        "UPDATE antiraid_sessions SET join_count = join_count + 1 WHERE id = $1", session_id
    )
