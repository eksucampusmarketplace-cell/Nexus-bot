...
async def set_exempted(
    ...
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow("""
            ...
            WHERE group_id = $1 AND user_id = $2
            RETURNING *
        """, group_id, user_id, exempted, exempted_by, reason)
        return dict(row) if row else None
...
async def get_boost_stats(group_id: int) -> dict:
    ...
    async with db.pool.acquire() as conn:
        ...
        total = await conn.fetchval(
            "SELECT COUNT(*) FROM member_boost_records WHERE group_id = $1",
            group_id
        )
...
async def create_credit_request(
    ...
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow("""
            ...
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *
        """, group_id, claimant_user_id, claimant_username, claimed_count, 
...
async def get_credit_request(group_id: int, request_id: int) -> Optional[dict]:
    ...
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT * FROM manual_add_credits WHERE group_id = $1 AND id = $2",
            group_id, request_id
        )
        return dict(row) if row else None
...
async def approve_credit_request(
    ...
    async with db.pool.acquire() as conn:
        ...
        """, group_id, request_id, amount, reviewed_by, note)
        ...
        await conn.execute("""
            ...
            SET manual_credits = member_boost_records.manual_credits + $4,
                updated_at = NOW()
        """, group_id, request['claimant_user_id'], request['claimant_username'], amount)
...
async def deny_credit_request(
    ...
    async with db.pool.acquire() as conn:
        await conn.execute("""
            ...
            WHERE group_id = $1 AND id = $2
        """, group_id, request_id, reviewed_by, reason)
...
async def assign_manual_add_credit(
    ...
    async with db.pool.acquire() as conn:
        ...
        """, group_id, added_user_id, credited_to_user_id)
        ...
        if "UPDATE 1" in result:
            ...
            await conn.execute("""
                ...
                ON CONFLICT (group_id, invited_user_id) DO NOTHING
            """, group_id, credited_to_user_id, added_user_id)
...
async def set_channel_restricted(group_id: int, user_id: int, restricted: bool = True) -> Optional[dict]:
...
async def save_boost_config(group_id: int, config: dict) -> bool:
    ...
    await upsert_group(group_id, group.get('title', 'Unknown') if group else 'Unknown', 
                      group.get('bot_token_hash', '') if group else '', settings)
    return True
