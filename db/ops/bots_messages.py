...
async def get_bot_custom_messages(bot_id: int) -> dict:
    ...
    async with db.pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT message_key, body FROM bot_custom_messages WHERE bot_id = $1",
            bot_id
        )
...
async def delete_bot_custom_message(bot_id: int, key: str):
    ...
    async with db.pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM bot_custom_messages WHERE bot_id = $1 AND message_key = $2",
            bot_id, key
        )
