...
async def upsert_pm(
    pool: asyncpg.Pool, bot_id: int, user_id: int, username: str = None, first_name: str = None
):
...
async def update_broadcast_progress(
    pool: asyncpg.Pool, task_id: int, sent_inc: int = 0, failed_inc: int = 0, status: str = None
):
    async with pool.acquire() as conn:
...
async def get_pending_broadcast_tasks(pool: asyncpg.Pool) -> List[dict]:
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT * FROM broadcast_tasks WHERE status = 'pending' ORDER BY created_at ASC"
        )
...
async def get_active_broadcast_tasks(pool: asyncpg.Pool) -> List[dict]:
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT * FROM broadcast_tasks WHERE status = 'running' ORDER BY created_at ASC"
        )
        return [dict(r) for r in rows]
    ...
    async with pool.acquire() as conn:
        ...
        if target_type in ("groups", "all"):
            ...
            bot = await conn.fetchrow(
                "SELECT token_hash, is_primary FROM bots WHERE bot_id = $1", bot_id
            )
            if bot and bot["is_primary"]:
                rows = await conn.fetch(
                    "SELECT chat_id FROM groups WHERE bot_token_hash = $1", bot["token_hash"]
                )
