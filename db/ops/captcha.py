...
async def create_challenge(
    db, chat_id, user_id, challenge_id, mode, answer, message_id, expires_at
):
    await db.execute(
...
async def get_challenge_by_id(db, challenge_id):
    return await db.fetchrow(
        "SELECT * FROM captcha_challenges WHERE challenge_id = $1", challenge_id
    )
...
async def mark_challenge_passed(db, challenge_id):
    await db.execute(
        "UPDATE captcha_challenges SET passed = TRUE WHERE challenge_id = $1", challenge_id
    )
