...
async def get_active_group_count(db: Connection, bot_id: int) -> int:
    ...
    row = await db.fetchrow(
        "SELECT COUNT(*) as c FROM clone_bot_groups " "WHERE bot_id=$1 AND is_active=TRUE", bot_id
    )
...
async def get_group_entry(db: Connection, bot_id: int, chat_id: int) -> dict | None:
    ...
    row = await db.fetchrow(
        "SELECT * FROM clone_bot_groups WHERE bot_id=$1 AND chat_id=$2", bot_id, chat_id
    )
    return dict(row) if row else None
...
async def update_access_status(db: Connection, bot_id: int, chat_id: int, status: str) -> None:
