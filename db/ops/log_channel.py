...
async def get_log_channel(pool, chat_id: int):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT log_channel_id FROM groups WHERE chat_id=$1", chat_id)
...
async def get_log_categories(pool, chat_id: int) -> dict:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT log_categories FROM groups WHERE chat_id=$1", chat_id)
