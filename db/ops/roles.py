...
PERMISSIONS = {
    # Moderation
    'warn_members', 'mute_members', 'kick_members',
    'ban_members', 'unban_members', 'purge_messages',
    # Content
    'pin_messages', 'post_channel', 'schedule_posts',
    # Admin
    'manage_roles', 'view_analytics', 'export_data',
    'manage_webhooks', 'manage_automod', 'manage_games',
}


async def create_role(chat_id: int, name: str, color: str = '#64748b', 
                       permissions: Dict = None) -> int:
    ...
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow(
            '''INSERT INTO roles (chat_id, name, color, permissions)
               ...
               RETURNING id''',
            chat_id, name, color, json.dumps(permissions or {})
        )
        return row['id']
...
async def delete_role(chat_id: int, role_id: int) -> bool:
    ...
    async with db.pool.acquire() as conn:
        result = await conn.execute(
            'DELETE FROM roles WHERE id = $1 AND chat_id = $2',
            role_id, chat_id
        )
...
async def get_role(chat_id: int, role_id: int) -> Optional[dict]:
    ...
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow(
            'SELECT * FROM roles WHERE id = $1 AND chat_id = $2',
            role_id, chat_id
        )
        if row:
            res = dict(row)
...
async def get_roles(chat_id: int) -> List[dict]:
    ...
    async with db.pool.acquire() as conn:
        rows = await conn.fetch(
            'SELECT * FROM roles WHERE chat_id = $1 ORDER BY name',
            chat_id
        )
        result = []
        for row in rows:
...
async def update_role_permissions(chat_id: int, role_id: int, 
    ...
    async with db.pool.acquire() as conn:
        result = await conn.execute(
            ...
            json.dumps(permissions), role_id, chat_id
        )
        return 'UPDATE 1' in result


async def assign_role(user_id: int, chat_id: int, role_id: int,
                       granted_by: int = None, expires_at=None) -> bool:
    ...
    async with db.pool.acquire() as conn:
        try:
            await conn.execute(
                ...
                user_id, chat_id, role_id, granted_by, expires_at
            )
            return True
        except Exception:
            return False
