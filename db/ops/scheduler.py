    ...
    async with pool.acquire() as conn:
        await conn.execute(
            """UPDATE scheduled_messages
               ...
               WHERE id=$4""",
            new_count, datetime.now(timezone.utc), next_send_at, msg_id
        )
...
async def deactivate_message(pool, msg_id: int):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE scheduled_messages SET is_active=FALSE WHERE id=$1",
            msg_id
        )
...
async def get_active_silent_times(pool) -> list:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """SELECT chat_id, slot, start_time, end_time, start_text, end_text
               FROM silent_times
               WHERE is_active = TRUE"""
        )
    return [dict(r) for r in rows]
