...
async def add_warn(user_id: int, chat_id: int, reason: str, by_user_id: int):
    ...
    async with db.pool.acquire() as conn:
        await conn.execute("UPDATE users SET warns = $1::jsonb WHERE user_id = $2 AND chat_id = $3", json.dumps(warns), user_id, chat_id)
    return len(warns)
...
async def update_user_status(user_id: int, chat_id: int, is_muted: bool = None, mute_until: datetime = None, is_banned: bool = None):
    updates = []
    params = []
