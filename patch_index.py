#!/usr/bin/env python3
"""Patch index.html to replace renderRolesPage with full CRUD implementation."""

content = open('miniapp/index.html').read()

# Find start of renderRolesPage
start_marker = '    async function renderRolesPage(container) {'
end_marker = '    async function renderBackupPage(container) {'

start_idx = content.index(start_marker)
end_idx = content.index(end_marker)

new_roles_code = '''    const ROLE_PERMISSIONS = [
...
new_content = content[:start_idx] + new_roles_code + content[end_idx:]
open('miniapp/index.html', 'w').write(new_content)
