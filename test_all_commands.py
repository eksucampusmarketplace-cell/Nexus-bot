...
def check_handler_functions():
    ...
    modules_to_check = [
        ...
        (
            "bot.handlers.pins",
            ["cmd_pin", "cmd_unpin", "cmd_unpinall", "cmd_repin", "cmd_editpin", "cmd_delpin"],
        ),
        ("bot.handlers.password", ["cmd_setpassword", "cmd_clearpassword", "handle_password_dm"]),
        (
            "bot.handlers.public",
            ["cmd_time", "cmd_kickme", "cmd_adminlist", "cmd_invitelink", "cmd_groupinfo"],
        ),
        ("bot.handlers.log_channel", ["cmd_setlog", "cmd_unsetlog", "cmd_logchannel"]),
        (
            "bot.handlers.import_export",
    ...
    for module_name, handler_names in modules_to_check:
        try:
            ...
            for handler_name in handler_names:
                issues["missing_handlers"].append(
                    (module_name, f"{handler_name} (module import failed)")
                )
                missing_count += 1
